...
export default function CaptureResultModal({ isOpen, onClose, data }) {
  if (!data) return null
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="xl" showClose={false}>
      ...
      <div className="text-center pb-4">
        <p className="text-lg font-semibold text-slate-200">{data.full_name}</p>
        <p className="text-sm text-slate-500">ID: {data.id_number}</p>
      </div>
    </Modal>
  )
}
